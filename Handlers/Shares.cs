...
{
    private Task Shares(Request req)
    {
        switch (req.Path)
        {
            case "/shares":
            { CreatePage(req, "Files", out var page, out var e, out var userProfile);
                ...
                if (req.Query.TryGetValue("u", out var u) && req.Query.TryGetValue("p", out var p))
                {
                    ...
                    e.Add(new ButtonElement(canEdit ? "View mode" : "View", null, $"@{user.Username}{(segments.Last().EndsWith(".wfpg") && segments.Last() != "index.wfpg" ? string.Join('/', ((IEnumerable<string>)[..segments.SkipLast(1), segments.Last()[..^5]]).Select(HttpUtility.UrlEncode)) : string.Join('/', p.Split('/').Select(HttpUtility.UrlEncode)))}", canEdit ? null : "green"));
                    if (!canEdit)
